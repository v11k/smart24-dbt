...
FROM 
    {{ source('google_search_console', 'search_analytics_by_date') }} s
LEFT JOIN 
    {{ ref("gsc_sites_with_attribute") }} a
    ON a.site_url = s.site_url
LEFT JOIN 
    {{ source('google_search_console', 'search_analytics_by_date') }} s_prev
    ON s.site_url = s_prev.site_url
    AND s.search_type = s_prev.search_type
    AND s.date = DATEADD(s_prev.date, INTERVAL 30 DAY)
WHERE 
